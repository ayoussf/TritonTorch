...
class SiLUTest(TritonKernelTest):
    ...
    def forward_triton(self, x: torch.Tensor) -> torch.Tensor:
        return self.silu_triton(x)

    def forward_torch(self, x: torch.Tensor) -> torch.Tensor:
        return self.silu_torch(x)

class TestSiLU:
    """
    Pytest test class for SiLU activation.
    """
    @pytest.fixture
    ...
    def test_silu(self,
                  ...
                  seed: int,):
        """
        Test SiLU forward and backward.
        """
        B, M, N = base_shape
        ...
        test = SiLUTest(config, B, M, N, dim)
        test.run()