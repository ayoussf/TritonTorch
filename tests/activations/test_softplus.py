...
from TritonTorch.Activations import Softplus
from tests.base import TritonKernelTest
from tests.utils import RunConfig, parametrize_dtypes, parametrize_dims

class SoftplusTest(TritonKernelTest):
    """
    Test class for Softplus activation.
    """
    ...
    def setup_modules(self) -> None:
        self.softplus_triton = Softplus()
        self.softplus_torch = nn.Softplus()
    ...
    def forward_triton(self, x: torch.Tensor) -> torch.Tensor:
        return self.softplus_triton(x)

    def forward_torch(self, x: torch.Tensor) -> torch.Tensor:
        return self.softplus_torch(x)

class TestSoftplus:
    """
    Pytest test class for Softplus activation.
    """
    @pytest.fixture
    ...
    def test_softplus(self,
                      ...
                      seed: int,):
        """
        Test Softplus forward and backward.
        """
        B, M, N = base_shape
        ...
        test = SoftplusTest(config, B, M, N, dim)
        test.run()