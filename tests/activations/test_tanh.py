...
class TanhTest(TritonKernelTest):
    ...
    def forward_triton(self, x: torch.Tensor) -> torch.Tensor:
        return self.tanh_triton(x)

    def forward_torch(self, x: torch.Tensor) -> torch.Tensor:
        return self.tanh_torch(x)

class TestTanh:
    """
    Pytest test class for Tanh activation.
    """
    @pytest.fixture
    ...
    def test_tanh(self,
                  ...
                  seed: int,):
        """
        Test Tanh forward and backward.
        """
        B, M, N = base_shape
        ...
        test = TanhTest(config, B, M, N, dim)
        test.run()
