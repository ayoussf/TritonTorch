...
from TritonTorch.Activations import LogSoftmax
from tests.base import TritonKernelTest
from tests.utils import RunConfig, parametrize_dtypes, parametrize_dims

class LogSoftmaxTest(TritonKernelTest):
    """
    Test class for LogSoftmax activation.
    """
    ...
    def setup_modules(self) -> None:
        self.logsoftmax_triton = LogSoftmax()
        self.logsoftmax_torch = nn.LogSoftmax(dim=-1)
    ...
    def forward_triton(self, x: torch.Tensor) -> torch.Tensor:
        return self.logsoftmax_triton(x)

    def forward_torch(self, x: torch.Tensor) -> torch.Tensor:
        return self.logsoftmax_torch(x)

class TestLogSoftmax:
    """
    Pytest test class for LogSoftmax activation.
    """
    @pytest.fixture
    ...
    def test_logsoftmax(self,
                        ...
                        seed: int,):
        """
        Test LogSoftmax forward and backward.
        """
        B, M, N = base_shape
        ...
        test = LogSoftmaxTest(config, B, M, N, dim)
        test.run()