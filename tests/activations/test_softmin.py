...
class SoftminTest(TritonKernelTest):
    """
    Test class for Softmin activation.
    """
    ...
    def forward_triton(self, x: torch.Tensor) -> torch.Tensor:
        return self.softmin_triton(x)

    def forward_torch(self, x: torch.Tensor) -> torch.Tensor:
        return self.softmin_torch(x)

class TestSoftmin:
    """
    Pytest test class for Softmin activation.
    """
    @pytest.fixture
    ...
    def test_softmin(self,
                     ...
                     device: str,
                     seed: int,):
        """Test Softmin forward and backward."""
        B, M, N = base_shape
        ...
        test = SoftminTest(config, B, M, N, dim)
        test.run()