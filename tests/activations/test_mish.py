...
class MishTest(TritonKernelTest):
    ...
    def forward_triton(self, x: torch.Tensor) -> torch.Tensor:
        return self.mish_triton(x)

    def forward_torch(self, x: torch.Tensor) -> torch.Tensor:
        return self.mish_torch(x)

class TestMish:
    """
    Pytest test class for Mish activation.
    """
    @pytest.fixture
    ...
    def test_mish(self,
                  ...
                  seed: int,):
        """
        Test Mish forward and backward.
        """
        B, M, N = base_shape
        ...
        test = MishTest(config, B, M, N, dim)
        test.run()