...
class LogSoftmaxUnitTest:
    def __init__(self, B=4, N=512, M=512, D=256, dtype=torch.float32, print_tb=False):
        ...
        # Triton logsoftmax and Torch logsoftmax
    ...
    def run(self):
        ...
        # interally tensors get flattened to 2D tensors (-1, C) before logsoftmax computation)
    ...
    def forward(self, input, input_ref, atol, rtol):
        ...
        self.start.record()
        output_ref = self.logsoftmax_torch(input_ref)
        self.end.record()
        torch.cuda.synchronize()
    ...
    for D in [32, 64, 128, 256, 512, 1024, 2048]:
        for i in range(2):
            ...
            for dtype in [torch.float16, torch.float32]:
                runner = LogSoftmaxUnitTest(B, N, M, D, dtype, print_tb)
                runner.run()
